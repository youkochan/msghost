...
exports.main = async (event, context) => {
  ...
  return db
    ...
    .group({
      ...
      winAsGhostCount: $.push(
        $.cond({
          if: $.and([$.eq(["$isGhost", true]), $.eq(["$isWinner", true])]),
          then: 1,
          else: 0,
        })
      ),
      winAsPlayerCount: $.push(
        $.cond({
          if: $.and([$.eq(["$isGhost", false]), $.eq(["$isWinner", true])]),
          then: 1,
          else: 0,
        })
      ),
      loseAsGhostCount: $.push(
        $.cond({
          if: $.and([$.eq(["$isGhost", true]), $.eq(["$isWinner", false])]),
          then: 1,
          else: 0,
        })
      ),
      loseAsPlayerCount: $.push(
        $.cond({
          if: $.and([$.eq(["$isGhost", false]), $.eq(["$isWinner", false])]),
          then: 1,
          else: 0,
        })
      ),
