...
exports.main = async (event, context) => {
  ...
  return Promise.all([p1, p2]).then(res => {
    ...
    const openids = playersData.map(i => i.openid).filter(i => i !== game._openid)
    ...
    const u1 = c1.where({ gameid: game._id, openid: db.command.in(d1) }).update({ data: { role: 1 } })
    const u2 = c1.where({ gameid: game._id, openid: db.command.in(d2) }).update({ data: { role: 2 } })
    const u3 = c1.where({ gameid: game._id, openid: db.command.in(d3) }).update({ data: { role: 3 } })
    const u4 = c2.doc(game._id).update({ data: { status: 1, majority: l1, minority: l2, ghost: l3 } })
