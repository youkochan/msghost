...
exports.main = async (event, context) => {
  ...
  return Promise.all([p1, p2]).then(res => {
    ...
    if (game.status !== 0) { throw Error('游戏已开始，无法加入') }

    if (openid === game._openid) { return db.collection('r_user_ghost').doc(rid).set({ data: { gameid: game._id, openid: openid, role: 0 }}) }
    else { return db.collection('r_user_ghost').doc(rid).set({ data: { gameid: game._id, openid: openid }}) }
  })
  .then(_ => { return { openid: wxContext.OPENID, gameid: event.id } })
